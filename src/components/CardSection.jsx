        ...
        >
          ...
          <p className="text-[15px] opacity-75 text-[#FFFFFFBF] font-[400] pb-3">
            {isLoading ? (
              ...
              !expandPositives && positives.length > 240
                ? positives?.slice(0, 240) + "..."
                : positives || ""
            )}
          </p>
          {!isLoading && positives.length > 240 && (
            <button
              onClick={() => setExpandPositives(!expandPositives)}
              disabled={isLoading}
              className="disabled:cursor-not-allowed disabled:opacity-40 flex items-center gap-1.5 font-[600] text-[#1482FF]"
            >
              <p>Learn More</p>
        ...
        >
          ...
          <p className="text-[15px] opacity-75 text-[#FFFFFFBF] font-[400] mb-4">
            {isLoading ? (
              ...
              !expandOpportunities && opportunities?.length > 240
                ? opportunities?.slice(0, 240) + "..."
                : opportunities || ""
            )}
          </p>
          {!isLoading && opportunities.length > 240 && (
            <button
              onClick={() => setExpandOpportunities(!expandOpportunities)}
              disabled={isLoading}
              className="disabled:cursor-not-allowed disabled:opacity-40 flex items-center gap-1.5 font-[600] text-[#1482FF]"
            >
              <p>Learn More</p>
